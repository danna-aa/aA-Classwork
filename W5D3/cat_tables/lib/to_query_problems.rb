...
def no_apples_for_blair
  ...
  execute(<<-SQL)
    ...
    WHERE
      toys.name = 'Apple' AND cats.name NOT LIKE 'Blair'
    ORDER BY
      cats.name ASC;
  SQL
end
...
def toys_that_jet_owns
  ...
  execute(<<-SQL)
    ...
    LEFT JOIN
          (SELECT
            toy_id, cats.name as cat_name, toys.name as toy_name
          FROM
            cattoys
          JOIN
            toys
          ...
          ON
            cats.id = cattoys.cat_id
          WHERE 
            cats.name != 'Jet') AS others
      ON
        jets_toys.toy_id = others.toy_id
      ...
      WHERE toy_id IN
        ...
        WHERE toys.id IN
          ...
          WHERE cat_id IN
            ...
            FROM cats
            WHERE cats.name = 'Jet')))
      AND cat_id != 1)  AS cat_toy_filtered
    LEFT JOIN cats
    ON cats.id = cat_id
    LEFT JOIN toys
    ON toys.id = toy_id
